...
const EQUIP_COLUMNS = [
  ...
  },
  {
    ...
    render: (r) => r.equipmentType || <span className="text-zinc-600">—</span>,
  },
  {
    key: 'serialNumber',
    label: 'Serial Number',
  ...
  },
  {
    key: 'office',
    label: 'Office',
    render: (r) => r.office || <span className="text-zinc-600">—</span>,
  },
]
...
const LOG_COLUMNS = [
  ...
  },
  {
    ...
    label: 'Notes',
    render: (r) => r.notes
      ? <span className="max-w-[220px] truncate block text-zinc-400" title={r.notes}>{r.notes}</span>
      : <span className="text-zinc-600">—</span>,
  },
  {
    key: 'createdAt',
    label: 'Date & Time',
    render: (r) => <span className="text-zinc-400 text-xs whitespace-nowrap">{formatDateTime(r.createdAt)}</span>,
  },
]
...
function Tracking() {
  ...
  const filteredEquip = inventoryItems.filter((item) => {
    ...
    return ['article', 'itemCode', 'equipmentType', 'serialNumber', 'office'].some(
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    return ['action', 'performedBy', 'location', 'notes'].some(
  ...
  return (
    <MainLayout>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 mb-5">
        <div className="px-5 py-3.5 border-b border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-zinc-300">Equipment Records</p>
            <p className="text-xs text-zinc-500 mt-px">Click a row to view its tracking history</p>
          </div>
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-zinc-700 bg-zinc-800 text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all duration-150"
            />
          </div>
        </div>
          ...
          {inventoryItems.length === 0 ? (
            ...
            <table className="min-w-full text-sm divide-y divide-zinc-800">
              <thead className="bg-zinc-900 sticky top-0 z-10">
                <tr>
                  {EQUIP_COLUMNS.map((c) => (
                    <th key={c.key} className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-500 uppercase tracking-wider whitespace-nowrap">
                      {c.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="bg-zinc-950 divide-y divide-zinc-800/60">
                {filteredEquip.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {EQUIP_COLUMNS.map((c) => (
                        <td key={c.key} className="px-4 py-3 text-zinc-300 whitespace-nowrap">
        ...
        <div className="px-5 py-3.5 border-b border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="min-w-0">
              ...
              {!logsLoading && (
                <p className="text-xs text-zinc-500 mt-px">{filteredLogs.length} {filteredLogs.length === 1 ? 'entry' : 'entries'}</p>
              )}
            </div>
            {selectedEquip && (
              <button
                onClick={handleViewAll}
                className="flex-shrink-0 text-xs text-zinc-500 hover:text-zinc-300 border border-zinc-700 hover:border-zinc-600 px-2.5 py-1 rounded-md transition-colors"
              >
                View All
              </button>
            )}

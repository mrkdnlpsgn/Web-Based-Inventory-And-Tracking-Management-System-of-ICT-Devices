        ...
        <div className="space-y-4">
          ...
          {inventoryItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-zinc-600">
              <p className="text-sm text-zinc-500">No inventory records found.</p>
              <p className="text-xs text-zinc-600">Add equipment in the Inventory module first.</p>
            </div>
              ...
              <table className="min-w-full text-sm divide-y divide-zinc-800">
                ...
                <tbody className="bg-zinc-950 divide-y divide-zinc-800/60">
                  {filteredEquip.map((item) => (
                    <tr
                      key={item.id}
                      onClick={() => { setSelectedEquip(item); setStep(2) }}
                      className="cursor-pointer hover:bg-zinc-800 transition-colors duration-100"
                    >
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="space-y-4">
            ...
            <Field label="Action" required error={errors.action}>
              <div className="relative">
                <select
                  ...
                  className={`w-full appearance-none rounded-md border px-3.5 py-2.5 pr-9 text-sm bg-zinc-800 text-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all duration-150 ${
                    errors.action ? 'border-red-500' : 'border-zinc-700 hover:border-zinc-600'
                  }`}
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Performed By" required error={errors.performedBy}>
                <TextInput
            ...
            <Field label="Notes">
              <textarea
                ...
                value={form.notes}
                onChange={set('notes')}
                className="w-full rounded-md border border-zinc-700 px-3.5 py-2.5 text-sm bg-zinc-800 text-white placeholder:text-zinc-600 hover:border-zinc-600 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all duration-150 resize-none"
              />
            </Field>
          </div>

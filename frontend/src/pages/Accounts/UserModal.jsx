...
function TextInput({ error, ...props }) {
  return (
    <input
      className={`w-full rounded-md border px-3.5 py-2.5 text-sm bg-zinc-800 text-white placeholder:text-zinc-600
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <Field label="Role" required error={errors.role}>
          <select
            value={form.role}
            onChange={set('role')}
            className="w-full rounded-md border border-zinc-700 px-3.5 py-2.5 text-sm bg-zinc-800 text-white
              hover:border-zinc-600 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500

  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs text-zinc-500">Manage staff and administrator accounts</p>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800">
        <div className="px-5 py-3.5 border-b border-zinc-800 flex items-center gap-3">
          <p className="text-sm font-semibold text-zinc-300">All Accounts</p>
          {users.length > 0 && (
              ...
              <thead>
                <tr>
                  {['Name', 'Email', 'Role', ''].map((h) => (
                    <th key={h} className="px-5 py-3 text-left text-[11px] font-semibold text-zinc-500 uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800/60">
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-zinc-800/40 transition-colors duration-100">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center flex-shrink-0">
                          <span className="text-xs font-semibold text-zinc-300">{user.name.charAt(0).toUpperCase()}</span>
                        </div>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setEditing(user)} title="Edit account"
                          className="p-1.5 rounded-md text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-all duration-150">
...
function AuditLogsTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-zinc-700 bg-zinc-900 text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
          />
        </div>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800">
        <div className="px-5 py-3.5 border-b border-zinc-800 flex items-center gap-3">
          <p className="text-sm font-semibold text-zinc-300">System Audit Logs</p>
          {!loading && (
            <span className="text-xs font-medium text-zinc-400 bg-zinc-800 border border-zinc-700 px-2 py-0.5 rounded-full">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm divide-y divide-zinc-800">
              ...
              <tbody className="divide-y divide-zinc-800/60">
                {filtered.map((log) => {
                  const aStyle = ACTION_STYLES[log.action]  || { bg: 'bg-zinc-700 text-zinc-300', label: log.action }
                  const mStyle = MODULE_STYLES[log.module] || { bg: 'bg-zinc-700 text-zinc-300' }
                  return (
                    <tr key={log.id} className="hover:bg-zinc-800/40 transition-colors duration-100">
                      ...
                      <td className="px-5 py-3.5 text-zinc-300 max-w-md">
                      ...
                      <td className="px-5 py-3.5 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-zinc-400 text-xs">{log.performedBy || 'System'}</span>
                        </div>
                      </td>
                    </tr>
                  )
    ...
    <MainLayout>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">User management and system audit trail</p>
        </div>
      </div>

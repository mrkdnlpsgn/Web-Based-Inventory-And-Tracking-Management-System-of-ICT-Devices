...
function DevicePicker({ devices, selected, onChange, lockedSerial = null }) {
  if (!devices.length) return null
  const allSelected = selected.length === devices.length
  const isLocked    = lockedSerial !== null
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        {isLocked && (
          <span className="text-xs text-slate-400 dark:text-zinc-500 italic">Locked to scanned device</span>
        )}
      </div>
      <div className="rounded-lg border border-slate-200 dark:border-zinc-700 divide-y divide-slate-100 dark:divide-zinc-800 max-h-44 overflow-y-auto">
        {devices.map((dev) => {
          const isThisLocked = isLocked && dev.serialNumber !== lockedSerial
          return (
            <label
              key={dev.id}
      ...
      <p className="text-xs text-slate-400 dark:text-zinc-600">
        {isLocked
          ? 'Only the scanned device is selected. Scan a different QR to log another device.'
          : 'Select which device(s) this log entry applies to.'}
      </p>
    </div>
  )
}
...
// initialSerialNumber  — pass to pre-select a specific device serial number (e.g. from scanned QR)
function LogEntryModal({ onClose, onSave, initialEquipment = null, initialSerialNumber = null }) {
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="space-y-4">
            ...
            <DevicePicker
              ...
              lockedSerial={initialSerialNumber}
            />

...
function buildQRCardHTML(equipment, device, svgHTML) {
  ...
  const code = equipment.itemCode || ''
  return `<div class="card">
    ${svgHTML}
    <p class="title">${equipment.article || '—'}</p>
    <p class="sub">${equipment.equipmentType || ''}</p>
    <p class="sub">${[equipment.office, equipment.location].filter(Boolean).join(' · ')}</p>
    ${sn}
...
function printDeviceQR(equipment, device, svgId) {
  ...
  <title>QR Code – ${equipment.article || equipment.itemCode}</title>
            ...
            <>
              ...
              <div className="w-full grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[400px] overflow-y-auto pr-1">
                {savedDevices.map((device, i) => {
                  const qrVal = device?.serialNumber
                    ? `${QR_PREFIX}${savedItem.id}:${device.serialNumber}`
                    : `${QR_PREFIX}${savedItem.id}`
              ...
              <div className="bg-white rounded-2xl p-6 w-full max-w-xs text-center shadow-lg">
                <div className="flex justify-center mb-4">
                  <QRCodeSVG
                    ...
                    value={savedDevices[0]?.serialNumber
                      ? `${QR_PREFIX}${savedItem.id}:${savedDevices[0].serialNumber}`
                      : `${QR_PREFIX}${savedItem.id}`}
                    size={160}
                    bgColor="#ffffff"
                    fgColor="#111827"
                ...
                <p className="text-base font-bold text-gray-900 leading-tight">{savedItem.article || '—'}</p>
                ...
                {savedDevices[0]?.serialNumber && (
                  <p className="text-xs font-mono text-gray-500 mt-1">S/N: {savedDevices[0].serialNumber}</p>
                )}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Type" hint="Broad category: Hardware, Software, Network Equipment, etc.">
                <SelectInput
                  name="type"
              ...
              <Field label="Type of Equipment" hint="The specific device type, e.g. Laptop, Printer, Router.">
                <SelectInput
                  name="equipmentType"
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <Field label="Accountable Person" hint="The staff member officially responsible for this equipment per property records.">
                <TextInput

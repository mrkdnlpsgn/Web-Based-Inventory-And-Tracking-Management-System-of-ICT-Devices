...
// Expand each equipment record into one flat row per device.
// Equipment with no devices produces one row with empty device fields.
function flattenEquipment(items) {
  const rows = []
  items.forEach((eq) => {
    ...
    if (devs.length > 0) {
      devs.forEach((dev) => {
        rows.push({
          ...
          accountablePerson: eq.accountablePerson,
          deviceCount:       eq.deviceCount,
          model:             dev.model,
          serialNumber:      dev.serialNumber,
          amountValue:       dev.amountValue,
          acquisitionDate:   dev.acquisitionDate,
        })
      })
    } else {
      rows.push({
...
function groupByType(items) {
  ...
  items.forEach((item) => {
    ...
    map[key].count += item.deviceCount > 0 ? item.deviceCount : 1
    const devTotal = (item.devices || []).reduce((s, d) => s + Number(d.amountValue || 0), 0)
    map[key].totalValue += devTotal
...
const REPORTS = [
  {
    ...
    async load() {
      return flattenEquipment((await getEquipment()).data)
    },
    headers: [
      ...
      { label: 'Article',          display: (r) => <span className="font-medium text-slate-900 dark:text-white">{r.article || '—'}</span>, raw: (r) => r.article || '' },
      ...
      { label: 'Description',      display: (r) => r.description ? <span className="max-w-[180px] truncate block" title={r.description}>{r.description}</span> : '—', raw: (r) => r.description || '' },
  ...
  },
  {
    ...
    headers: [
      { label: 'Equipment Type',   display: (r) => <span className="font-medium text-slate-900 dark:text-white">{r.equipmentType}</span>, raw: (r) => r.equipmentType },
  ...
  },
  {
    ...
    headers: [
      { label: 'Article',          display: (r) => <span className="font-medium text-slate-900 dark:text-white">{r.article || '—'}</span>, raw: (r) => r.article || '' },
      { label: 'Item Code',        display: (r) => <span className="font-mono text-xs">{r.itemCode || '—'}</span>,       raw: (r) => r.itemCode || '' },
      { label: 'Serial Number(s)', display: (r) => r.serialNumbers ? <span className="font-mono text-xs text-blue-400">{r.serialNumbers}</span> : '—', raw: (r) => r.serialNumbers || '' },
      { label: 'Equipment Type',   display: (r) => r.equipmentType || '—',                                                raw: (r) => r.equipmentType || '' },
  ...
  },
  {
    ...
    async load() {
      ...
      return flattenEquipment(items)
        .filter((r) => r.amountValue != null && r.amountValue !== '')
...
function ValuationTotalsRow({ rows }) {
  ...
  return (
    <tr className="border-t-2 border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800/60">
      <td className="px-4 py-3 text-sm font-bold text-slate-900 dark:text-white" colSpan={4}>Total</td>
...
function Reports() {
  ...
  const getExportRows = () => {
    ...
    return [
      ...reportData,
      {
        article: 'TOTAL ACQUISITION VALUE', type: '', itemCode: '', equipmentType: '',
        model: '', serialNumber: '', amountValue: total, acquisitionDate: '',
            ...
            >
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-150 ${
                isActive
                  ...
                  : `${report.iconBg} ${report.iconColor} group-hover:bg-brand-500/10 group-hover:text-brand-400`
              }`}>
                {report.icon}
              </div>
                    ...
                    <tr>
                      {Array.from({ length: 5 }).map((_, i) => (
                        <th key={i} className="px-4 py-3">
                          <div className="animate-pulse h-2.5 w-16 rounded bg-slate-200 dark:bg-zinc-700" />
                        </th>
                      ))}
                    </tr>
                  </thead>
                      ...
                      <tr key={i}>
                        {Array.from({ length: 5 }).map((_, j) => (
                          <td key={j} className="px-4 py-3">
                            <div className="animate-pulse h-3 rounded bg-slate-200 dark:bg-zinc-800" style={{ width: `${60 + (j * 10) % 40}%` }} />
                          </td>
                        ))}
                      </tr>
                    ))}

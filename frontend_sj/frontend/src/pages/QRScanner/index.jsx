...
function QRScanner() {
  ...
  const processCode = useCallback((raw) => {
    // Strip prefix, then take only the equipment-ID segment (handles "id:serialNumber" format)
    const stripped = raw.startsWith(QR_PREFIX) ? raw.slice(QR_PREFIX.length) : raw.trim()
  ...
  return (
    <MainLayout>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 overflow-hidden">
            <div className="px-5 py-3.5 border-b border-slate-200 dark:border-zinc-800">
              ...
              <p className="text-xs text-slate-400 dark:text-zinc-500 mt-px">Point your camera at an ICT device QR code</p>
            </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-5">
            ...
            <p className="text-xs text-slate-400 dark:text-zinc-500 mb-3">Type an Item Code to search directly.</p>
        ...
        <div>
          ...
          {notFound && (
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-red-200 dark:border-red-800/40 p-8 flex flex-col items-center justify-center gap-3 min-h-[320px] animate-fade-slide">
              ...
              <p className="text-sm font-semibold text-red-500 dark:text-red-400">Equipment Not Found</p>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 overflow-hidden animate-fade-slide">
              <div className="px-5 py-3.5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-slate-700 dark:text-zinc-300">Equipment Found</p>
                </div>
                <div className="flex items-center gap-2">
      ...
      <div className="mt-5 bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800">
        <div className="px-5 py-3.5 border-b border-slate-200 dark:border-zinc-800 flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
          <p className="text-sm font-semibold text-slate-700 dark:text-zinc-300">Hardware Scanner — Mockup Handshake</p>
          <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 ring-1 ring-amber-200 dark:ring-amber-500/20 font-medium">Demo Mode</span>
        </div>
        <div className="p-5 space-y-4">
          <p className="text-xs text-slate-400 dark:text-zinc-500">
            Simulates a physical barcode/QR scanner device (ID: <span className="font-mono text-slate-500 dark:text-zinc-400">{mockDeviceId}</span>) sending scan data directly to the Business Tier via <span className="font-mono text-slate-500 dark:text-zinc-400">POST /api/scanner/receive</span>. This proves the integration endpoint is architecturally ready to receive hardware input.
          </p>
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3.5 py-2 text-sm text-slate-700 dark:text-zinc-200 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all font-mono"
            />
            <button
            ...
            <div className={`rounded-lg border p-4 text-sm font-mono space-y-1 ${
              mockResponse.status === 'OK'
                ...
                : mockResponse.status === 'NOT_FOUND'
                  ? 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-700/40'
                  : 'bg-slate-50 dark:bg-zinc-800 border-slate-200 dark:border-zinc-700'
            }`}>
              <p className="text-xs text-slate-400 dark:text-zinc-500 mb-2">Business Tier Response:</p>
              <p>
                <span className="text-slate-400 dark:text-zinc-500">status: </span>
                <span className={
                  mockResponse.status === 'OK' ? 'text-emerald-600 dark:text-emerald-400 font-semibold' :
                  mockResponse.status === 'NOT_FOUND' ? 'text-red-500 dark:text-red-400 font-semibold' :
                  'text-amber-600 dark:text-amber-400 font-semibold'
                }>{mockResponse.status}</span>
              </p>
              <p><span className="text-slate-400 dark:text-zinc-500">message: </span><span className="text-slate-600 dark:text-zinc-300">{mockResponse.message}</span></p>
              {mockResponse.equipment && (
                <p><span className="text-slate-400 dark:text-zinc-500">equipment: </span>
                  <span className="text-emerald-600 dark:text-emerald-300">{mockResponse.equipment.article} ({mockResponse.equipment.itemCode})</span>
                </p>
              )}
            </div>
          )}
